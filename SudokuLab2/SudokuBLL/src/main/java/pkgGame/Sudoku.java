...
public class Sudoku extends LatinSquare {
	...
	public int getRegionNbr(int iRow, int iCol) {
	 ...
	 private void setRegion(int reg) {
		 int val = 1;
		 for (int i = (reg / iSqrtSize) * iSqrtSize; i < (reg / iSqrtSize) * iSqrtSize + iSqrtSize; i++) {
			 for (int j = (reg % iSqrtSize) * iSqrtSize; j < (reg % iSqrtSize) * iSqrtSize + iSqrtSize; j++) {
				 getPuzzle()[i][j] = val++;
				 }
			 }
		 }
	 ...
	 private void shuffleRegion(int reg) {
		 int[] regionArr = getRegion(reg);
		 shuffleArray(regionArr);
		 int val = 0;
		  
		 for (int i = (reg / iSqrtSize) * iSqrtSize; i < (reg / iSqrtSize) * iSqrtSize + iSqrtSize; i++) {
			 for (int j = (reg % iSqrtSize) * iSqrtSize; j < (reg % iSqrtSize) * iSqrtSize + iSqrtSize; j++) {
				 getPuzzle()[i][j] = regionArr[val++];
				 }
			 }
		 }
